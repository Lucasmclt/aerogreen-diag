...
def build_recommended_actions(scores, data):
    ...
    if scores["Achats responsables"] < 70:
        actions.append("Renforcer les achats responsables : critères RSE fournisseurs, cartographie des risques, preuves de certification et suivi fournisseur.")
    if scores["REEN"] < 70:
...
def create_deep_audit_pdf(company, diagnostic_result, data, scores):
    ...
    pdf_key_values(pdf, "1. Informations entreprise", {
        ...
        "Objectif EcoVadis vise": data["meta"]["target_level"],
    ...
    pdf_key_values(pdf, "5. EcoVadis - Environnement", data["environment"])
    pdf_key_values(pdf, "Indicateurs environnementaux", data["environment_metrics"])
    pdf.paragraph(data.get("environment_notes", ""))

    pdf_key_values(pdf, "6. EcoVadis - Social", data["social"])
    pdf_key_values(pdf, "Indicateurs sociaux", data["social_metrics"])
    pdf.paragraph(data.get("social_notes", ""))

    pdf_key_values(pdf, "7. EcoVadis - Ethique", data["ethics"])
    pdf_key_values(pdf, "8. EcoVadis - Achats responsables", data["procurement"])
...
def render_deep_audit():
    ...
    <div class="page-header">
        <div class="page-header-tag">Dossier approfondi</div>
        <h2>Structurer un dossier RSE numérique approfondi.</h2>
        <div class="page-header-sub">
            Cette section vise à structurer un dossier de travail, pas à produire une évaluation officielle : preuves documentaires, REEN, CAO/PLM, achats responsables et données carbone.
        </div>
    </div>
    """, unsafe_allow_html=True)

    if not st.session_state.get("diagnostic_done") or not st.session_state.get("diagnostic_result"):
        st.warning("Lancez d’abord le diagnostic avancé afin d’avoir une base indicative de score et de synthèse.")
        if st.button("Aller au diagnostic avancé", use_container_width=True):
    ...
    if not current_code or not audit_public_code_exists(st.session_state.user_id, current_code):
        st.warning("Aucun diagnostic enregistré n’est rattaché à ce dossier approfondi. Enregistrez d’abord le diagnostic depuis la page Score.")
    ...
    st.info("Cette section vise à structurer un dossier de travail, pas à produire une évaluation officielle.")

    with st.expander("0. Cadrage de la pré-analyse", expanded=True):
        c1, c2, c3 = st.columns(3)
        with c1:
            target_level = st.selectbox("Objectif de préparation EcoVadis", ["Bronze", "Argent", "Or", "Platine", "Non défini"], key="deep_target_level")
        with c2:
    ...
    with st.expander("1. Documents à rassembler", expanded=True):
        docs = render_checkbox_group([
            ...
            "Certificats ISO 14001 / ISO 26000 / ISO 45001 ou équivalents",
    ...
    with st.expander("2. EcoVadis - Environnement", expanded=False):
    ...
    with st.expander("3. EcoVadis - Social", expanded=False):
    ...
    with st.expander("4. EcoVadis - Éthique", expanded=False):
        ...
        ethics_notes = st.text_area("Notes éthique / conformité", height=130, key="ethics_notes")

    with st.expander("5. EcoVadis - Achats responsables", expanded=False):
        procurement = render_checkbox_group([
            "Critères RSE dans la sélection fournisseurs",
            "Demande de certifications fournisseurs",
        ...
        with c2:
            audited_suppliers = st.number_input("% fournisseurs audités", 0.0, 100.0, 0.0, key="audited_suppliers")
        with c3:
    ...
    data = {
        ...
        "procurement_metrics": {
            ...
            "% fournisseurs audités": audited_suppliers,
    ...
    c3.metric("EcoVadis env.", f"{scores['Environnement']:.0f}/100")
    ...
    with st.container(border=True):
        ...
        with c_pdf:
            st.download_button(
                "Télécharger le rapport approfondi PDF",
                data=pdf_bytes,
                file_name=f"AeroGreen_Rapport_Approfondi_{datetime.now().strftime('%Y%m%d')}.pdf",
