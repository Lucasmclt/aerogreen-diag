...
def render_report():
    ...
    if not saved_audits:
        st.warning("Aucun diagnostic enregistré n’est disponible pour générer un rapport de travail.")
    ...
    if current_code:
        for idx, label in enumerate(labels):
            if audit_options[label] == current_code:
                default_index = idx
                break
    selected_label = st.selectbox("Diagnostic enregistré à exporter", labels, index=default_index)
    ...
    if selected_audit is None:
        st.warning("Le diagnostic sélectionné n’existe plus. Retournez au dashboard ou relancez un diagnostic avancé.")
        return
    ...
    st.download_button(
        label="Télécharger le rapport de travail PDF",
        data=pdf_bytes,
