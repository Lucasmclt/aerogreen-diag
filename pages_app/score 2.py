...
def render_score():
    render_page_header(
        "Score AeroGreen",
        "Lecture synthétique indicative issue du questionnaire déclaratif.",
        "Scoring indicatif"
    )
    ...
    with st.container(border=True):
        ...
        <div class='ag-bottom-card-header'>
            ...
            <div class='feature-text small'>Enregistrez ce cas, générez un rapport de travail ou structurez un dossier RSE numérique approfondi.</div>
        </div>
        ...
        with cta3:
            if st.button("Compléter le dossier RSE", use_container_width=True):
                ...
                if not audit_id:
                    return
                st.session_state.page = "Audit approfondi"
                st.query_params["page"] = "Audit approfondi"
                st.rerun()
